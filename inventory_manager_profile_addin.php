 ...
 * @throws AdmException
 */
function insertKeeperView(HtmlPage $page, User $user, CItems $itemsKeeper): void
{
	...
	');
    $page->addJavascript('
        $("#inventory_manager_box_keeper").insertBefore("#profile_roles_box")
    ');
}
 ...
 * @throws AdmException
 */
function insertReceiverView(HtmlPage $page, User $user, CItems $itemsReceiver): void
{
    global $gL10n, $gCurrentOrgId;
    ...
    $addinItemFields = array('ITEMNAME');
    foreach ($pPreferences->config['Optionen']['profile_addin'] as $addinField) {
        // we are in the last receiver view, so we don't need the last receiver field in the table
	...
	');
    $page->addJavascript('
        $("#inventory_manager_box_receiver").insertBefore("#profile_roles_box")
    ');
}
